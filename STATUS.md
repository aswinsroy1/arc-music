...
- Visual Polish: Dynamically updated status bar contrast to ensure icons remain visible over bright album art glow colors. Standardized bottom nav icons to Material Outlined and softened the active tab pill opacity for better visual balance. Reduced Home screen "Arc Music" header size to `displaySmall` to minimize visual competition with the hero card.
- Added explicit-content tracking via the Deezer API. Rebuilt the `tracks` Room table to include a nullable `isExplicit` field. The `fetchMissingArtwork` background loop now additionally checks tracks without explicit status, queries Deezer, and updates the local database. UI conditionally displays an "E" badge next to the track title in the Now Playing screen and Library lists.
